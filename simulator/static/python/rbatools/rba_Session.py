...
class SessionRBA(object):
    ...
    def __init__(self, xml_dir: str,lp_solver="cplex"):
        ...
        self.ExchangeMap = build_exchange_map(RBA_Session=self)
    ...
    def record_results(self, run_name: str):
        ...
        for i in Exchanges.keys():
            self.Results['ExchangeFluxes'].loc[i, run_name] = check_solution_feasibility(
                Value=Exchanges[i], RBA_Session=self)

        self.Results['Reactions'][run_name] = [check_solution_feasibility(
            Value=self.Problem.SolutionValues[i], RBA_Session=self) for i in list(self.Results['Reactions'].index)]
        self.Results['Enzymes'][run_name] = [check_solution_feasibility(
            Value=self.Problem.SolutionValues[i], RBA_Session=self) for i in list(self.Results['Enzymes'].index)]
        self.Results['Processes'][run_name] = [check_solution_feasibility(
            Value=self.Problem.SolutionValues[i], RBA_Session=self) for i in list(self.Results['Processes'].index)]
        self.Results['Constraints'][run_name] = [check_solution_feasibility(
            Value=self.Problem.DualValues[i], RBA_Session=self) for i in self.Problem.LP.row_names]
        self.Results['Proteins'][run_name] = record_proteome(RBA_Session=self, run=run_name)
        self.Results['ProtoProteins'][run_name] = record_proto_proteome(RBA_Session=self, run=run_name, Proteinlevels=self.Results['Proteins'])
        self.Results['SolutionType'][run_name] = check_solution_feasibility(Value=self.Problem.SolutionType, RBA_Session=self)
        self.Results['Mu'][run_name] = check_solution_feasibility(Value=self.Problem.Mu, RBA_Session=self)
        self.Results['ObjectiveValue'][run_name] = check_solution_feasibility(
            Value=self.Problem.ObjectiveValue, RBA_Session=self)
        self.Results['ObjectiveFunction'][run_name] = list(self.Problem.get_objective().values())
    ...
    def write_results(self, session_name: str, digits: int = 10):
        ...
        if hasattr(self, 'Results'):
            self.Results['uniqueReactions'] = map_iso_reactions(RBA_Session=self)
    ...
    def solve(self, run_name: str = 'DontSave', feasible_stati: list = ["optimal","feasible"], try_unscaling_if_sol_status_is_feasible_but_not_optimal: bool = True) -> bool:
        """
        ...
        try_unscaling_if_sol_status_is_feasible_but_not_optimal : bool
        ...
        self.Problem.solve_lp(feasible_stati=feasible_stati, try_unscaling_if_sol_status_is_feasible_but_not_optimal=try_unscaling_if_sol_status_is_feasible_but_not_optimal)
    ...
    def find_max_growth_rate(self, precision: float = 0.001, max: float = 4.0, start_value: float = numpy.nan, recording: bool = False, omit_objective: bool = False, feasible_stati: list = ["optimal","feasible"], try_unscaling_if_sol_status_is_feasible_but_not_optimal: bool = True) -> float:
        """
    ...
    def find_min_substrate_concentration(self, metabolite: str, precision: float =0.00001, max: float =100.0, recording:bool =False):
        """
    ...
    def get_feasible_range(self, variables: Union[list,str]=[]) -> dict:
        """
    ...
    def get_pareto_front(self, variable_X: str, variable_Y: str, N: int = 10, sign_VY: str = 'max') -> pandas.core.frame.DataFrame:
        ...
        if variable_X not in self.Problem.LP.col_names:
            raise InputError('Chosen Element for variable X not among problem variables')
            #warnings.warn('Chosen Element for variable X not among problem variables')
            #return
        if variable_Y not in self.Problem.LP.col_names:
            raise InputError('Chosen Element for variable Y not among problem variables')
            #warnings.warn('Chosen Element for variable Y not among problem variables')
            #return
        FR = self.get_feasible_range(variable_X)
        ...
        oldLB = self.Problem.get_lb(variable_X)
        oldUB = self.Problem.get_ub(variable_X)
        iteration = -1
        for conc in concentrations:
            iteration += 1
            self.Problem.set_lb(inputDict={variable_X: conc})
            self.Problem.set_ub(inputDict={variable_X: conc})
            self.Problem.clear_objective()
            if sign_VY == 'max':
                self.Problem.set_objective(inputDict={variable_Y: -1})
            if sign_VY == 'min':
                self.Problem.set_objective(inputDict={variable_Y: 1})
            self.Problem.solve_lp()
            if self.Problem.Solved:
    ...
    def get_current_parameter_value(self, parameter: str) -> float:
        ...
        try:
            ...
            term_results=[evaluate_current_function_value(expression_dictionary=i,growth_rate=self.Mu , medium=self.Medium) for i in list_param_definitions_to_evaluate]
    ...
    def get_parameter_definition(self, parameter: str) -> dict:
        ...
        """
        return(return_parameter_definition(model=self.model,parameter=parameter))
    ...
    def get_model_statistics_information(self) -> dict:
        ...
        """
        return(return_model_statistics_information(model_structure=self.ModelStructure))
    ...
    def get_general_model_information(self) -> dict:
        ...
        """
        return(return_general_model_information(model_structure=self.ModelStructure))
    ...
    def get_compartment_information(self,compartment: str) -> dict:
        ...
        """
        return(return_compartment_information(model_structure=self.ModelStructure,compartment=compartment))
    ...
    def get_enzyme_information(self,enzyme: str) -> dict:
        ...
        """
        return(return_enzyme_information(model_structure=self.ModelStructure,enzyme=enzyme))
    ...
    def get_protein_information(self,protein: str) -> dict:
        ...
        """
        return(return_protein_information(model_structure=self.ModelStructure,protein=protein))
    ...
    def get_reaction_information(self,reaction: str) -> dict:
        ...
        """
        return(return_reaction_information(model_structure=self.ModelStructure,reaction=reaction))
    ...
    def get_metabolite_information(self,metabolite: str) -> dict:
        ...
        """
        return(return_metabolite_information(model_structure=self.ModelStructure,metabolite=metabolite))
    ...
    def get_process_information(self,process: str) -> dict:
        ...
        """
        return(return_process_information(model_structure=self.ModelStructure,process=process))
    ...
    def get_macro_molecule_information(self,macro_molecule: str) -> dict:
        ...
        """
        return(return_macro_molecule_information(model_structure=self.ModelStructure,macro_molecule=macro_molecule))
    ...
    def get_target_information(self,target: str) -> dict:
        ...
        """
        return(return_target_information(model_structure=self.ModelStructure,target=target))
    ...
    def get_module_information(self,module: str) -> dict:
        ...
        Dictionary with information
        """
        return(return_module_information(model_structure=self.ModelStructure,module=module))
    ...
    def get_density_constraint_information(self,density_constraint: str) -> dict:
        ...
        """
        return(return_density_constraint_information(model_structure=self.ModelStructure,density_constraint=density_constraint))
    ...
    def get_process_constraint_information(self,process_constraint: str) -> dict:
        ...
        Dictionary with information:
            ...
            'CapacityParameter' : Definition of capacity parameter (type dict)
                See doc string of rbatools.rba_session.SessionRBA.get_parameter_definition method
        """
        return(return_process_constraint_information(model_structure=self.ModelStructure,process_constraint=process_constraint))
    ...
    def get_metabolite_constraint_information(self,metabolite_constraint: str) -> dict:
        ...
        """
        return(return_metabolite_constraint_information(model_structure=self.ModelStructure,metabolite_constraint=metabolite_constraint))
    ...
    def get_enzyme_constraint_information(self,enzyme_constraint: str) -> dict:
        ...
        Dictionary with information:
           ...
           'CapacityParameter' : Definition of capacity parameter (type dict)
                See doc string of rbatools.rba_session.SessionRBA.get_parameter_definition method
        """
        return(return_enzyme_constraint_information(model_structure=self.ModelStructure,enzyme_constraint=enzyme_constraint))
    ...
    def get_compartments(self) -> list:
        ...
        List of IDs
        """
        return(return_compartments(model_structure=self.ModelStructure))
    ...
    def get_enzymes(self) -> list:
        ...
        List of IDs
        """
        return(return_enzymes(model_structure=self.ModelStructure))
    ...
    def get_proteins(self) -> list:
        ...
        List of IDs
        """
        return(return_proteins(model_structure=self.ModelStructure))
    ...
    def get_reactions(self) -> list:
        ...
        List of IDs
        """
        return(return_reactions(model_structure=self.ModelStructure))
    ...
    def get_metabolites(self) -> list:
        ...
        List of IDs
        """
        return(return_metabolites(model_structure=self.ModelStructure))
    ...
    def get_processes(self) -> list:
        ...
        List of names
        """
        return(return_processes(model_structure=self.ModelStructure))
    ...
    def get_macro_molecules(self) -> list:
        ...
        List of IDs
        """
        return(return_macro_molecules(model_structure=self.ModelStructure))
    ...
    def get_targets(self) -> list:
        ...
        List of IDs
        """
        return(return_targets(model_structure=self.ModelStructure))
    ...
    def get_modules(self) -> list:
        ...
        List of IDs
        """
        return(return_modules(model_structure=self.ModelStructure))
    ...
    def get_density_constraints(self) -> list:
        ...
        List of IDs
        """
        return(return_density_constraints(model_structure=self.ModelStructure))
    ...
    def get_process_constraints(self) -> list:
        ...
        List of IDs
        """
        return(return_process_constraints(model_structure=self.ModelStructure))
    ...
    def get_metabolite_constraints(self) -> list:
        ...
        List of IDs
        """
        return(return_metabolite_constraints(model_structure=self.ModelStructure))
            ...
            if parameter_definition[parameter]['Variables'][0] in x_values.keys():
                if parameter_definition[parameter]['Variables'][0]=="growth_rate":
                    ...
                    if parameter_definition[parameter]['Variables'][0] in medium.keys():
                        ...
                        #warnings.warn("X variable not found")
            else:
                raise Exception("Variable not found in x values")
                #warnings.warn("Variable not found in x values")
        else:
