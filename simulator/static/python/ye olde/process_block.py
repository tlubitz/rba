...
def getProcessInfo(model, i):
    ...
    initiationString = ''
    if len(ProcessingMaps[i].constant_processing.reactants._elements) > 0:
        for j in range(len(ProcessingMaps[i].constant_processing.reactants._elements)):
            initiationString = initiationString + ' ' + \
                str(int(ProcessingMaps[i].constant_processing.reactants._elements[j].stoichiometry)) + ' ' + \
                ProcessingMaps[i].constant_processing.reactants._elements[j].species  # int() added#
    initiationString = initiationString + ' -->'
    if len(ProcessingMaps[i].constant_processing.products._elements) > 0:
        for j in range(len(ProcessingMaps[i].constant_processing.products._elements)):
            initiationString = initiationString + ' ' + \
                str(int(ProcessingMaps[i].constant_processing.products._elements[j].stoichiometry)) + ' ' + \
                ProcessingMaps[i].constant_processing.products._elements[j].species  # int() added#
    if initiationString == ' -->':
