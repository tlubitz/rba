...
from rbastructure.rba_LP import RBA_LP
...
#        self.MuDepIndices_Adict = {i: (self.LP.rowIndicesMap[i[0]], self.LP.colIndicesMap[i[1]]) for i in self.MuDepIndices_A}
    ...
    def setMu_modifiedProblem(self, Mu, ModelStructure, keepParameters=None, logging=True):
        ...
        inputMatrix.A = scipy.sparse.coo_matrix(scipy.sparse.lil_matrix(self.MuOneMatrix.A)*self.Mu)
        ...
        for coeff in ModelStructure.MuDependencies:
            if coeff in list(ModelStructure.EnzymeConstraintsInfo.Elements.keys()):
                ...
                rowName = coeff
                colName = ModelStructure.ProcessConstraintsInfo.Elements[coeff]['AssociatedProcess']
        ...
        value = determineCoefficient(x=parameters, changes={'Mu': self.Mu}, species='Mu')
        if coeff in list(ModelStructure.DensityConstraintsInfo.Elements.keys()):
            self.setRighthandSideValue(inputDict={rowName: value}, logging=logging)
        elif coeff in list(ModelStructure.EnzymeConstraintsInfo.Elements.keys()):
            self.setProblemCoefficients(
                inputDict={rowName: {colName: -value}}, logging=logging)
